...
try:
    from .nmf import reconstruct
except ImportError:
    warnings.warn("no NMF-C implementation found")
...
class NMFMatrix:
    ...
    def compute(self, niters=25):
        for niter in range(niters):
            ...
            try:
                reconstructed = reconstruct(self.matrix.row, self.matrix.col, self.matrix.data, self.W, self.H)
            except NameError:
                reconstructed = self.__reconstruct()
